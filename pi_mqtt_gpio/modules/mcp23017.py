...
class GPIO(GenericGPIO):
    ...
    def setup_pin(self, pin, direction, pullup, pin_config):
        ...
        if direction == PinDirection.INPUT and pullup is not None:
            pair = self.io.get_offset_gpio_tuple([0x0C, 0x0D], pin)
            self.io.set_bit_enabled(pair[0], pair[1], PULLUPS[pullup])

        initial = pin_config.get("initial")
        if initial is not None:
            if initial == "high":
                self.io.digital_write(pin, 0xFF)
            elif initial == "low":
                self.io.digital_write(pin, 0x00)
